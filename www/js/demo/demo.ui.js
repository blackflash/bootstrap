(function(a){a(document).ready(function(c){
	...
	a("#da-ex-accordion, #da-ex-accordion-plain").accordion();a("#da-ex-tabs, #da-ex-tabs-plain").tabs();

	a("#da-ex-datepicker, #da-ex-datepicker-inline").datepicker({showOtherMonths:true});a("#da-ex-datepicker-week").datepicker({showOtherMonths:true,showWeek:true});
	a("#da-ex-datepicker-months").datepicker({showOtherMonths:true,numberOfMonths:3});a("#da-ex-datetimepicker").datetimepicker();a("#da-ex-datetimepicker2").datetimepicker();
	a("#da-ex-timepicker").timepicker({});a(".da-ex-buttons").button();a("#da-ex-buttons-radio, #da-ex-buttons-checkbox").buttonset();
	a("#da-ex-buttons-icon button:first").button({icons:{primary:"ui-icon-locked"},text:false}).next().button({icons:{primary:"ui-icon-locked"}}).next().button({icons:{primary:"ui-icon-gear",secondary:"ui-icon-triangle-1-s"}}).next().button({icons:{primary:"ui-icon-gear",secondary:"ui-icon-triangle-1-s"},text:false});var b=["ActionScript","AppleScript","Asp","BASIC","C","C++","Clojure","COBOL","ColdFusion","Erlang","Fortran","Groovy","Haskell","Java","JavaScript","Lisp","Perl","PHP","Python","Ruby","Scala","Scheme"];a("#da-ex-autocomplete").autocomplete({source:b});
	a("#da-ex-slider").slider();a("#da-ex-slider-range").slider({range:true,min:0,max:500,values:[75,300],slide:function(d,e){
	...
	a("#da-ex-slider-range-fixed").slider({range:"min",value:1,min:1,max:100,slide:function(d,e){
	a("#da-ex-slider-range-fixed-info .da-highlight").html(e.value)}});a("#da-slider-equalizer > span").each(function(){var d=parseInt(
		a(this).text(),10);a(this).empty().slider({value:d,range:"min",animate:true,orientation:"vertical"})});

	a("#da-ex-slider-range-fixed3").slider({range:"min",value:57.5,min:2009,max:2016,slide:function(d,e){
	a("#da-ex-slider-range-fixed-info3 .da-highlight").html(e.value)}});a("#da-slider-equalizer > span").each(function(){var d=parseInt(
		a(this).text(),10);a(this).empty().slider({value:d,range:"min",animate:true,orientation:"vertical"})});

	a("#da-ex-slider-range-fixed2").slider({range:"min",value:1,min:1,max:100,slide:function(d,e){
	a("#da-ex-slider-range-fixed-info2 .da-highlight").html(e.value)}});a("#da-slider-equalizer > span").each(function(){var d=parseInt(
		a(this).text(),10);a(this).empty().slider({value:d,range:"min",animate:true,orientation:"vertical"})});
