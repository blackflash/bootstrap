...
class Auth extends CI_Controller
{
	...
	function login($language = "")
	{
		if ($this->tank_auth->is_logged_in()) {									// logged in
			...
			$user_profile = $this->notes_model->getRowById2("user_profiles","user_id",$this->tank_auth->get_user_id());
			...
			if($language == "sk")
			redirect($language."/index/domov/notActivated");
			else
			redirect($language."/index/home/notActivated");		
			...
			$this->form_validation->set_rules('remember', 'Zapamätať', 'integer');
			...
			if ($this->form_validation->run()) {
				if ($this->tank_auth->login(
					...
					if($user_profile->user_type == "company"){
						...
						redirect('sk/loadPage/profil_uzivatel');
					}
			...
			$data["banner"] = $this->notes_model->getTable("banner");

			if($language == "sk"){
				$titleName  = "prihlásiť sa"; 
				$loginImage  = "loginSK.png";
				$this->lang->load('cleverfrogs', 'slovak');
			}
			
			else{
				$titleName = "login" ;
				$loginImage  = "loginEN.png";
				$this->lang->load('cleverfrogs', 'english');
			} 

			$data = array(
				'login' => $loginImage, 
				'title' => "Cleverfrogs ".$titleName, 
				'logo' => "logo.png",
				'view' => "auth/login_form.php",
				'language' => $language
			);
	 ...
	 * @return void
	 */
	function register($language = "sk", $redirect = "")
	{	
		$errors = "";
		...
		if ($this->tank_auth->is_logged_in()) {		
			...
			if ($this->form_validation->run()) {								// validation ok
				if (!is_null($data = $this->tank_auth->create_user(
						...
						$this->form_validation->set_value('password'),
						$email_activation,$activated))) {									// success
					...
					if ($email_activation) {									// send "activate" email
						...
						redirect($language."/index/domov/registerSuccess");
	...
	function reset_password()
	{
		...
		$this->form_validation->set_rules('new_password', 'Nové heslo', 'trim|required|xss_clean|min_length['.$this->config->item('password_min_length', 'tank_auth').']|max_length['.$this->config->item('password_max_length', 'tank_auth').']|alpha_dash');
		$this->form_validation->set_rules('confirm_new_password', 'Potvrdenie nového hesla', 'trim|required|xss_clean|matches[new_password]');
	...
	{	
		...
		if (!$this->tank_auth->is_logged_in()) {								// not logged in or not activated
			...
			$this->form_validation->set_rules('old_password', 'Staré heslo', 'trim|required|xss_clean');
			$this->form_validation->set_rules('new_password', 'Nové heslo', 'trim|required|xss_clean|min_length['.$this->config->item('password_min_length', 'tank_auth').']|max_length['.$this->config->item('password_max_length', 'tank_auth').']|alpha_dash');
			$this->form_validation->set_rules('confirm_new_password', 'Potvrdenie nového hesla', 'trim|required|xss_clean|matches[new_password]');
